...
silver_total AS (
    ...
    FROM ecommerce_lakehouse.Silver.order_payments op
    WHERE EXISTS (
        SELECT 1
        FROM ecommerce_lakehouse.Silver.order_items oi
...
SELECT
    ...
    ROUND(ABS(f.fact_revenue - s.silver_revenue)
          / NULLIF(s.silver_revenue, 0) * 100, 4)           AS variance_pct,
    CASE
        WHEN ABS(f.fact_revenue - s.silver_revenue)
             / NULLIF(s.silver_revenue, 0) * 100 <= 1.0
        THEN 'PASS' ELSE 'FAIL'
    END                                                     AS result
FROM fact_total f
CROSS JOIN silver_total s; -- both CTEs return one aggregate row